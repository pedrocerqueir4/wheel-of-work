...
import { Zap, BrainCircuit, Coffee, Palette } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useAppStore } from "@/store/appStore";
import type { Task, WheelMode } from "@shared/types";
...
const getPath = (startAngle: number, endAngle: number) => {
  const start = {
    x: 50 + 48 * Math.cos(startAngle),
    y: 50 + 48 * Math.sin(startAngle),
  };
  const end = {
    x: 50 + 48 * Math.cos(endAngle),
    y: 50 + 48 * Math.sin(endAngle),
  };
...
export function WheelDisplay() {
  ...
  const filteredTasks = useMemo(() => {
    ...
    switch (wheelMode) {
      ...
      case 'time-to-work':
        return tasks.filter(t => t.category === 'work' || t.category === 'creative');
      case 'normal':
      default:
        return tasks;
    }
    ...
    <Card className="rounded-2xl shadow-soft overflow-hidden">
      <CardContent className="p-6 md:p-8 flex flex-col items-center justify-center space-y-6">
        <ToggleGroup
          type="single"
          value={wheelMode}
          onValueChange={(value: WheelMode) => value && setWheelMode(value)}
          className="w-full justify-center"
          aria-label="Wheel Mode"
        >
          <ToggleGroupItem value="hard-working" aria-label="Hard-working mode">
            <BrainCircuit className="h-4 w-4 mr-2" /> Hard-Working
          </ToggleGroupItem>
          <ToggleGroupItem value="time-to-work" aria-label="Time-to-work mode">
            <Palette className="h-4 w-4 mr-2" /> Time-to-Work
          </ToggleGroupItem>
          <ToggleGroupItem value="normal" aria-label="Normal mode">
            <Coffee className="h-4 w-4 mr-2" /> Normal
          </ToggleGroupItem>
        </ToggleGroup>
        ...
        <Button
          size="lg"
          className="px-10 py-6 text-lg font-bold rounded-full shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-200 active:scale-95 bg-brand-pink hover:bg-brand-pink/90 text-white"
          onClick={handleSpin}
